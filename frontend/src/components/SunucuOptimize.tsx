...
export default function SunucuOptimize() {
  const { t } = useTranslation(['SunucuOptimize', 'common'])
  ...
  async function baslat() {
    ...
    try {
      ...
      setLog(t('SunucuOptimize:started_log'))
      setCalisiyor(true)
    } catch (e: any) {
      setHata(e?.response?.data?.hata || e?.message || t('SunucuOptimize:started_failed'))
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-baseline gap-2">
            <span className="text-sm font-semibold text-slate-900 dark:text-slate-100">{t('SunucuOptimize:title')}</span>
            <span className="text-[10px] uppercase tracking-wider px-1.5 py-0.5 rounded font-medium bg-sky-100 dark:bg-sky-900/40 text-sky-700 dark:text-sky-300">{t('SunucuOptimize:badge')}</span>
          </div>
          <div className="text-xs text-slate-500 dark:text-slate-500 mt-0.5">
            {t('SunucuOptimize:desc')}
          </div>
          ...
          <div className="mt-3 flex items-center gap-2">
            {!onay ? (
              <button onClick={() => setOnay(true)} disabled={calisiyor || baslatiliyor}
                ...
                {t('SunucuOptimize:start_button')}
              </button>
            ) : (
              <>
                <span className="text-xs text-slate-600 dark:text-slate-300">{t('SunucuOptimize:confirm_text')}</span>
                <button onClick={baslat} disabled={baslatiliyor}
                  ...
                  {baslatiliyor ? t('SunucuOptimize:starting') : t('SunucuOptimize:yes_start')}
                </button>
