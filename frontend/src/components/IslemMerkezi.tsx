...
export default function IslemMerkezi({ admin }: { admin: boolean }) {
  ...
  const temizle=useCallback(async()=>{setYukleniyor(true);setHata('');try{await api.delete('/islemler');await Promise.all([listeYukle(),ozetYukle()])}catch(e){setHata(apiHata(e,t('operations.clear_failed')));setYukleniyor(false)}},[listeYukle,ozetYukle,t])
  ...
  return <div className="relative">
    ...
    {acik&&<><button type="button" aria-label={t('operations.close')} className="fixed inset-0 z-40 cursor-default" onClick={()=>setAcik(false)}/><section className="fixed right-2 top-20 z-50 mt-1 max-h-[min(75vh,40rem)] w-[calc(100vw-1rem)] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl dark:border-slate-700 dark:bg-slate-900 sm:absolute sm:right-0 sm:top-full sm:w-[28rem]">
      <header className="flex items-center justify-between border-b border-slate-100 px-4 py-3 dark:border-slate-800"><div><h2 className="text-sm font-semibold dark:text-white">{t('operations.title')}</h2><p className="text-xs text-slate-500">{t('operations.subtitle')}</p></div><div className="flex items-center gap-1"><button onClick={()=>void temizle()} disabled={yukleniyor||!islemler.some(x=>!['bekliyor','calisiyor'].includes(x.durum))} className="rounded-lg px-2 py-1 text-xs text-slate-500 hover:bg-slate-100 disabled:opacity-40 dark:hover:bg-slate-800">{t('operations.clear')}</button><button onClick={()=>void listeYukle()} disabled={yukleniyor} className="rounded-lg px-2 py-1 text-xs text-brand-600 hover:bg-brand-50 disabled:opacity-50">{t('operations.refresh')}</button></div></header>
