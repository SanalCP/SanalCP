...
export default function IzlemePage() {
  const { t } = useTranslation(['IzlemePage', 'common'])
  const [tab, setTab] = useState<'sunucu' | 'domain' | 'loglar'>('sunucu')
  return (
    <div className="px-6 py-5">
      <Breadcrumb items={[{ etiket: t('common:home'), href: '/' }, { etiket: t('IzlemePage:breadcrumb') }]} />
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-2xl font-semibold text-slate-900 dark:text-slate-100">{t('IzlemePage:title')}</h1>
        <span className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-500">
          <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
          {t('IzlemePage:live', { n: POLL_MS / 1000 })}
        </span>
      </div>

      <div className="flex gap-1 mb-5 border-b border-slate-200 dark:border-slate-700">
        <SekmeButon aktif={tab === 'sunucu'}  onClick={() => setTab('sunucu')}>{t('IzlemePage:tabs.server')}</SekmeButon>
        <SekmeButon aktif={tab === 'domain'} onClick={() => setTab('domain')}>{t('IzlemePage:tabs.domain')}</SekmeButon>
        <SekmeButon aktif={tab === 'loglar'} onClick={() => setTab('loglar')}>{t('IzlemePage:tabs.logs')}</SekmeButon>
      </div>
    ...
    <>
      ...
      {u && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
          <Snap baslik={t('IzlemePage:server.snap.cpu')} deger={u.cpu.yuzde.toFixed(1) + '%'} alt={t('IzlemePage:server.snap.cpu_cores', { n: u.cpu.cekirdek })} renk="indigo" />
          <Snap baslik={t('IzlemePage:server.snap.mem')} deger={u.bellek.yuzde.toFixed(1) + '%'}
            alt={`${(u.bellek.kullanilan_kb/1024).toFixed(0)} / ${(u.bellek.toplam_kb/1024).toFixed(0)} MB`} renk="emerald" />
          <Snap baslik={t('IzlemePage:server.snap.load')} deger={u.cpu.yuk_1dk.toFixed(2)}
            alt={t('IzlemePage:server.snap.load_detail', { m5: u.cpu.yuk_5dk.toFixed(2), m15: u.cpu.yuk_15dk.toFixed(2) })} renk="amber" />
          <Snap baslik={t('IzlemePage:server.snap.disk')} deger={u.disk.yuzde.toFixed(1) + '%'}
            alt={`${fmtByte(u.disk.kullanilan_byte)} / ${fmtByte(u.disk.toplam_byte)}`} renk="violet" />
        </div>
      )}

      {/* Çok serili çizgi grafik */}
      <Kart baslik={t('IzlemePage:server.resources_title')} sag={t('IzlemePage:server.resources_sample', { count: noktalar.length, max: MAX_NOKTA, min: (noktalar.length*POLL_MS/1000/60).toFixed(1) })}>
        <div className="flex items-center gap-4 mb-2 text-xs">
          <Lej renk="bg-indigo-500" et={t('IzlemePage:server.legend.cpu')} />
          <Lej renk="bg-emerald-500" et={t('IzlemePage:server.legend.mem')} />
          <Lej renk="bg-violet-500" et={t('IzlemePage:server.legend.swap')} />
          <Lej renk="bg-amber-500" et={t('IzlemePage:server.legend.load')} />
        </div>
      ...
      {/* Ağ trafiği grafiği */}
      <Kart baslik={t('IzlemePage:server.network_title')} sag={u?.ag?.arayuz ? t('IzlemePage:server.network_iface', { iface: u.ag.arayuz }) : ''}>
        <div className="flex items-center gap-4 mb-2 text-xs">
          <Lej renk="bg-sky-500" et={t('IzlemePage:server.legend.rx')} />
          <Lej renk="bg-pink-500" et={t('IzlemePage:server.legend.tx')} />
        </div>
        <CokSeriliGrafik
      ...
      {/* Servisler */}
      {u && (
        <Kart baslik={t('IzlemePage:server.services_title')} sag={t('IzlemePage:server.services_active', { active: u.servisler.filter(s => s.aktif).length, total: u.servisler.length })}>
            ...
            {u.servisler.map(s => (
              <div key={s.ad} className={`flex items-center gap-2 px-3 py-2 rounded-md border text-xs ${
                ...
                <span className={`text-[10px] font-semibold uppercase ${s.aktif ? 'text-emerald-700 dark:text-emerald-300' : 'text-red-700 dark:text-red-300'}`}>
                  {s.aktif ? t('IzlemePage:server.service_active') : t('IzlemePage:server.service_off')}
                </span>
              </div>
            ))}
          </div>
        ...
        <div className="lg:overflow-x-auto">
          <table className={T.tablo}>
            <thead className={`${T.baslikGrubu} border-b border-slate-200 dark:border-slate-700`}>
              <tr>
                <th className={T.baslik}>{t('IzlemePage:server.table.pid')}</th>
                <th className={T.baslik}>{t('IzlemePage:server.table.user')}</th>
                <th className={`${T.baslik} text-right`}>{t('IzlemePage:server.table.cpu')}</th>
                <th className={`${T.baslik} text-right`}>{t('IzlemePage:server.table.mem')}</th>
                <th className={T.baslik}>{t('IzlemePage:server.table.cmd')}</th>
              </tr>
            </thead>
...
function DomainIzleme() {
  ...
  return (
    <>
      <Kart baslik={t('IzlemePage:domain.select_title')}>
        <div className="flex items-center gap-3 flex-wrap">
          <select value={secili ?? ''} onChange={e => setSecili(Number(e.target.value))}
            ...
            {domains.length === 0 && <option value="">{t('IzlemePage:domain.no_active_domain')}</option>}
          ...
          {secili && (
            <button onClick={() => probet(secili)} disabled={hSorgulaniyor}
              ...
              {hSorgulaniyor ? t('IzlemePage:domain.probing') : t('IzlemePage:domain.probe_btn')}
            </button>
          )}
          {seciliDomain && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        <Kart baslik={t('IzlemePage:domain.access_log')} sag={t('IzlemePage:domain.last_lines', { n: accessLog.length })}>
          <div ref={accessRef} className="bg-slate-950 text-emerald-300 font-mono text-[11px] p-3 rounded h-80 overflow-auto whitespace-pre">
            {accessLog.length === 0
              ? <div className="text-slate-500 dark:text-slate-500 italic">{t('IzlemePage:domain.no_log_yet')}</div>
              : accessLog.join('\n')}
          </div>
        </Kart>
        <Kart baslik={t('IzlemePage:domain.error_log')} sag={t('IzlemePage:domain.last_lines', { n: errorLog.length })}>
          <div ref={errorRef} className="bg-slate-950 text-rose-300 font-mono text-[11px] p-3 rounded h-80 overflow-auto whitespace-pre">
            {errorLog.length === 0
              ? <div className="text-slate-500 dark:text-slate-500 italic">{logHata || t('IzlemePage:domain.no_error_yet')}</div>
              : errorLog.join('\n')}
          </div>
        </Kart>
      </div>
...
function SunucuLoglari() {
  ...
  const LOG_KAYNAK_ET: Record<string, string> = {
    panel: t('IzlemePage:logs.sources.panel'), nginx: t('IzlemePage:logs.sources.nginx'), mariadb: t('IzlemePage:logs.sources.mariadb'), named: t('IzlemePage:logs.sources.named'),
    sshd: t('IzlemePage:logs.sources.sshd'), cron: t('IzlemePage:logs.sources.cron'), sistem: t('IzlemePage:logs.sources.sistem'),
  }
    ...
    <div>
      <div className="flex flex-wrap items-center gap-2 mb-3">
        ...
        <div className="ml-auto flex items-center gap-2">
          <input value={arama} onChange={e => setArama(e.target.value)} placeholder={t('IzlemePage:logs.search_placeholder')}
          ...
          <select value={son} onChange={e => setSon(Number(e.target.value))}
            ...
            {[100, 200, 500, 1000].map(n => <option key={n} value={n}>{t('IzlemePage:logs.last_n', { n })}</option>)}
          </select>
          <button onClick={() => yukle()} disabled={yuk} className="px-3 py-1.5 text-xs border border-slate-200 dark:border-slate-700 rounded-md text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 disabled:opacity-50">{t('IzlemePage:logs.refresh')}</button>
        </div>
      </div>
      {hata && <div className="mb-3 px-3 py-2 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-md text-sm text-red-700 dark:text-red-300">{hata}</div>}
      <div ref={scrollRef} className="bg-slate-900 border border-slate-800 rounded-2xl overflow-auto p-3 font-mono text-xs leading-relaxed whitespace-pre-wrap break-all" style={{ height: 560 }}>
        {yuk ? <div className="text-slate-500 py-4">{t('common:loading')}</div>
          : gorunen.length === 0 ? <div className="text-slate-500 py-4">{arama ? t('IzlemePage:logs.not_found', { q: arama }) : t('IzlemePage:logs.no_records')}</div>
            : gorunen.map((s, i) => <div key={i} className={logRenk(s)}>{s}</div>)}
      </div>
      <p className="text-xs text-slate-400 mt-2">{arama
        ? t('IzlemePage:logs.footer_filtered', { count: gorunen.length, total: satirlar.length, source: LOG_KAYNAK_ET[kaynak] || kaynak })
        : t('IzlemePage:logs.footer', { count: satirlar.length, source: LOG_KAYNAK_ET[kaynak] || kaynak })}</p>
    </div>
  )
}
...
function CokSeriliGrafik({
  ...
  const { t } = useTranslation(['IzlemePage'])
  const W = 1000, H = 180, P = 8
  const innerW = W - P * 2, innerH = H - P * 2
  if (noktalar.length < 2) {
    return <div className="text-xs text-slate-400 dark:text-slate-500 italic py-12 text-center">{t('IzlemePage:server.chart_collecting')}</div>
  }
    ...
    <div className={`rounded-2xl p-4 border ${ok ? 'border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-900/20' : 'border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-900/20'}`}>
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className={`text-sm font-semibold ${ok ? 'text-emerald-800 dark:text-emerald-200' : 'text-red-800 dark:text-red-200'}`}>
          {ok ? t('IzlemePage:domain.health.reachable') : t('IzlemePage:domain.health.unreachable')}
        </span>
      </div>
  ...
  return (
    <div className={`rounded-2xl p-4 border ${krit ? 'border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-900/20' : 'border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-900/20'}`}>
      <div className={`text-sm font-semibold mb-2 ${krit ? 'text-red-800 dark:text-red-200' : 'text-emerald-800 dark:text-emerald-200'}`}>
        {ssl.gecerli ? t('IzlemePage:domain.ssl.valid') : t('IzlemePage:domain.ssl.invalid')}
      </div>
      <div className="text-2xl font-bold text-slate-900 dark:text-slate-100 font-mono">{ssl.kalan_gun}<span className="text-base ml-1 text-slate-500 dark:text-slate-500">{t('IzlemePage:domain.ssl.days')}</span></div>
      <div className="text-[11px] text-slate-500 dark:text-slate-500 mt-1">{t('IzlemePage:domain.ssl.expires')} <span className="font-mono">{ssl.bitis_tarihi}</span></div>
      {ssl.cikaran && <div className="text-[11px] text-slate-500 dark:text-slate-500">{t('IzlemePage:domain.ssl.issuer')} <span className="font-mono">{ssl.cikaran}</span></div>}
    </div>
  )
}
function YanitKart({ h }: { h: Health }) {
  ...
  return (
    <div className={`rounded-2xl p-4 border ${m[renk]}`}>
      <div className="text-sm font-semibold mb-2">{t('IzlemePage:domain.response.title')}</div>
      ...
      <div className="text-[11px] text-slate-500 dark:text-slate-500 mt-1">
        {ms < 300 ? t('IzlemePage:domain.response.fast') : ms < 1000 ? t('IzlemePage:domain.response.acceptable') : t('IzlemePage:domain.response.slow')} · {h.sema.toUpperCase()}
      </div>
    </div>
  )
}
