...
export default function DomainErisimKontrolPage() {
  ...
  async function hotlinkKaydet(aktif: boolean) {
    ...
    try {
      ...
      setOk(aktif ? t('DomainErisimKontrolPage:hotlink.enabled_msg') : t('DomainErisimKontrolPage:hotlink.disabled_msg'))
      yukle()
    } catch (e) { setHata(apiHata(e, t('DomainErisimKontrolPage:hotlink.save_failed'))) }
  ...
  async function ipModKaydet(mod: IPMod) {
    ...
    try {
      ...
      setIpMod(mod)
      setOk(t('DomainErisimKontrolPage:ip.mode_updated'))
      yukle()
    } catch (e) { setHata(apiHata(e, t('DomainErisimKontrolPage:ip.save_failed'))) }
  ...
  async function ipEkle(e: React.FormEvent) {
    ...
    try {
      ...
      setIpYeni('')
      yukle()
    } catch (e2) { setHata(apiHata(e2, t('DomainErisimKontrolPage:ip.add_failed'))) }
    finally { setIpKaydediyor(false) }
  }
  ...
  async function ipSil(k: IPKural) {
    ...
    catch (e) { setHata(apiHata(e, t('DomainErisimKontrolPage:ip.delete_failed'))) }
  }
  ...
  return (
    <div className="px-6 py-5">
      ...
      <div className="flex items-center gap-3 mb-1">
        ...
        <h1 className="text-xl font-semibold text-slate-900 dark:text-slate-100">{t('DomainErisimKontrolPage:title')}</h1>
      </div>
      <p className="text-sm text-slate-500 dark:text-slate-400 mb-5">{t('DomainErisimKontrolPage:subtitle')}</p>
        ...
        <>
          <div className="bg-white dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/60 rounded-2xl p-4 mb-5">
            <div className="flex items-center justify-between mb-1">
              <h3 className="text-[11px] uppercase tracking-wide text-slate-400 font-semibold">{t('DomainErisimKontrolPage:hotlink.title')}</h3>
              <span className={`text-[10px] font-semibold uppercase tracking-wider px-1.5 py-0.5 rounded ${hotlink.aktif ? 'text-emerald-700 dark:text-emerald-300 bg-emerald-100 dark:bg-emerald-900/30' : 'text-slate-500 bg-slate-100 dark:bg-slate-700/50'}`}>
                {hotlink.aktif ? t('DomainErisimKontrolPage:hotlink.active') : t('DomainErisimKontrolPage:hotlink.inactive')}
              </span>
            </div>
            <p className="text-xs text-slate-500 dark:text-slate-400 mb-3">
              {t('DomainErisimKontrolPage:hotlink.desc')}
            </p>
            <label className="block mb-3">
              <span className="text-[11px] uppercase tracking-wide text-slate-400 font-semibold">{t('DomainErisimKontrolPage:hotlink.allowed_label')}</span>
              <input value={hotlinkIzinliMetin} onChange={e => setHotlinkIzinliMetin(e.target.value)} placeholder={t('DomainErisimKontrolPage:hotlink.allowed_placeholder')}
            ...
            <button onClick={() => hotlinkKaydet(!hotlink.aktif)} disabled={hotlinkKaydediyor}
              ...
              {hotlinkKaydediyor ? t('DomainErisimKontrolPage:hotlink.saving') : hotlink.aktif ? t('DomainErisimKontrolPage:hotlink.disable') : t('DomainErisimKontrolPage:hotlink.enable')}
            </button>
            {hotlink.aktif && (
              <button onClick={() => hotlinkKaydet(true)} disabled={hotlinkKaydediyor} className="ml-2 px-4 py-2 border border-slate-300 dark:border-slate-600 text-sm rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 disabled:opacity-50">
                {t('DomainErisimKontrolPage:hotlink.update_list')}
              </button>
            )}
          </div>

          <div className="bg-white dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/60 rounded-2xl p-4">
            <h3 className="text-[11px] uppercase tracking-wide text-slate-400 font-semibold mb-1">{t('DomainErisimKontrolPage:ip.title')}</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 mb-3">{t('DomainErisimKontrolPage:ip.desc')}</p>
            <div className="flex flex-wrap gap-2 mb-3">
              {(['kapali', 'engelle', 'izin_ver'] as IPMod[]).map(m => (
                <button key={m} onClick={() => ipModKaydet(m)} disabled={ipKaydediyor}
                  ...
                  {m === 'kapali' ? t('DomainErisimKontrolPage:ip.mode_off') : m === 'engelle' ? t('DomainErisimKontrolPage:ip.mode_block') : t('DomainErisimKontrolPage:ip.mode_allow')}
                </button>
              ))}
            </div>
              ...
              <>
                <form onSubmit={ipEkle} className="flex items-end gap-2 mb-3">
                  <label className="block flex-1">
                    <span className="text-[11px] uppercase tracking-wide text-slate-400 font-semibold">{t('DomainErisimKontrolPage:ip.cidr_label')}</span>
                    <input value={ipYeni} onChange={e => setIpYeni(e.target.value)} required placeholder={t('DomainErisimKontrolPage:ip.cidr_placeholder')}
                      className="mt-1 w-full px-3 py-2 border border-slate-300 dark:border-slate-600 dark:bg-slate-900 rounded-lg text-sm font-mono focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 outline-none" />
                  </label>
                  <button disabled={ipKaydediyor || !ipYeni.trim()} className="px-4 py-2 bg-slate-900 hover:bg-slate-800 dark:bg-white dark:hover:bg-slate-100 text-white dark:text-slate-900 text-sm font-medium rounded-lg disabled:opacity-50">
                    {t('DomainErisimKontrolPage:ip.add')}
                  </button>
                </form>
                {ipListe.length === 0 ? (
                  <p className="text-sm text-slate-500 dark:text-slate-400 py-2">
                    {ipMod === 'izin_ver' ? t('DomainErisimKontrolPage:ip.empty_allow') : t('DomainErisimKontrolPage:ip.empty_block')}
                  </p>
                ) : (
                  <ul className="divide-y divide-slate-100 dark:divide-slate-700/60">
                    {ipListe.map(k => (
                      <li key={k.id} className="flex items-center justify-between py-2">
                        ...
                        <button onClick={() => ipSil(k)} className="text-xs text-red-600 dark:text-red-400 hover:underline">{t('DomainErisimKontrolPage:ip.delete')}</button>
                      </li>
                    ))}
                  </ul>
                )}
      ...
      <div className="mt-4"><Link to={`/abonelikler/${id}`} className="text-sm text-brand-600 dark:text-brand-400">{t('DomainErisimKontrolPage:back_to_subscription')}</Link></div>
    </div>
  )
}
