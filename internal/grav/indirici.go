...
const releaseAPI = "https://api.github.com/repos/getgrav/grav/releases/latest"
...
var kararliSurum = regexp.MustCompile(`^2\.[0-9]+\.[0-9]+$`)
...
func sonSurum(ctx context.Context) (releaseBilgi, error) {
	...
	if err != nil {
		return releaseBilgi{}, fmt.Errorf("Grav sürüm bilgisi alınamadı: %w", err)
	}
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		return releaseBilgi{}, fmt.Errorf("Grav sürüm bilgisi alınamadı: GitHub API HTTP %d", resp.StatusCode)
	}
	var body struct {
	...
	if err := json.NewDecoder(io.LimitReader(resp.Body, 2<<20)).Decode(&body); err != nil {
		return releaseBilgi{}, fmt.Errorf("Grav sürüm bilgisi ayrıştırılamadı: %w", err)
	}
	if body.Draft || body.Prerelease || !kararliSurum.MatchString(body.Tag) {
		return releaseBilgi{}, fmt.Errorf("kararlı Grav 2 sürümü bulunamadı")
	}
	for _, a := range body.Assets {
		if a.Name == "grav-admin-v"+body.Tag+".zip" && strings.HasPrefix(a.URL, "https://api.github.com/repos/getgrav/grav/releases/assets/") &&
	...
	return releaseBilgi{}, fmt.Errorf("doğrulanabilir Grav Admin2 paketi bulunamadı")
}
...
func indirVeDogrula(ctx context.Context, hedef string) (string, error) {
	...
	resp, err := http.DefaultClient.Do(req)
	if err != nil {
		return "", fmt.Errorf("Grav paketi indirilemedi: %w", err)
	}
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		return "", fmt.Errorf("Grav paketi indirilemedi: HTTP %d", resp.StatusCode)
	}
	tmp, err := os.CreateTemp("", "grav-*.zip")
	if err != nil {
		return "", err
	}
	ad := tmp.Name()
	...
	closeErr := tmp.Close()
	if cpErr != nil {
		return "", fmt.Errorf("Grav paketi yazılamadı: %w", cpErr)
	}
	if closeErr != nil {
		return "", closeErr
	}
	if n != rel.Boyut {
		return "", fmt.Errorf("Grav paket boyutu uyuşmuyor: beklenen %d, gelen %d", rel.Boyut, n)
	}
	if hex.EncodeToString(h.Sum(nil)) != rel.SHA256 {
		return "", fmt.Errorf("Grav paket SHA-256 doğrulaması başarısız")
	}
...
func zipAc(arsiv, hedef string) error {
	...
	if err != nil {
		return fmt.Errorf("Grav zip açılamadı: %w", err)
	}
	defer zr.Close()
	...
	for _, f := range zr.File {
		...
		if filepath.IsAbs(temiz) || temiz == ".." || strings.HasPrefix(temiz, "../") {
			return fmt.Errorf("güvensiz Grav zip girdisi: %s", f.Name)
		}
		rel, ok := strings.CutPrefix(temiz, "grav-admin/")
		if !ok || rel == "" || rel == "." {
			continue
		}
		...
		if f.Mode()&os.ModeSymlink != 0 {
			return fmt.Errorf("Grav zip sembolik bağlantı içeriyor: %s", f.Name)
		}
		if f.FileInfo().IsDir() {
		...
		if dosyaSayisi > 100000 || toplam > 512<<20 {
			return fmt.Errorf("Grav paketi açma sınırını aşıyor")
		}
		...
		if cpErr != nil || n != int64(f.UncompressedSize64) {
			return fmt.Errorf("Grav zip girdisi eksik: %s", f.Name)
		}
		if outErr != nil {
			return outErr
		}
	...
	if _, err := os.Stat(filepath.Join(kok, "bin", "plugin")); err != nil {
		return fmt.Errorf("Grav paketi plugin CLI dosyasını içermiyor")
	}
	return nil
}
