...
func wafDomainConfYaz(sk, engine string, paranoya int) error {
	...
	if _, err := os.Stat(custom); err != nil {
		_ = os.WriteFile(custom,
			[]byte("# SanalCP WAF — "+sk+" ozel kurallar / haric-tutmalar (opsiyonel, MVP: bos).\n"+
				"# Ornek CRS istisnasi: SecRuleRemoveById 942100\n"), 0644)
	}
	var b strings.Builder
	b.WriteString("# SanalCP WAF — " + sk + " — OTOMATIK URETILDI, elle duzenlemeyin.\n")
...
func buildModSec(sk string) string {
	...
	if !WAFModulYuklu() {
		log.Printf("waf: %s WAF-etkin ama ModSecurity modulu yuklu DEGIL — WAF ATLANDI (vhost saglam). 'sanalcp-waf-setup' calistirin.", sk)
		return ""
	}
...
func HealWAFOnStartup() {
	...
	if aktifSayi > 0 && !modul {
		log.Printf("waf heal: %d WAF-etkin domain var ama ModSecurity modulu YUKLU DEGIL — "+
			"'sanalcp-waf-setup' calistirilmali (WAF su an PASIF, vhost'lar saglam)", aktifSayi)
	} else {
