...
// doğrular (eksikse yalnız uyarı loglar — sanalcp-mail-setup henüz çalıştırılmamış
...
func HealMailOnStartup(ctx context.Context, db *sql.DB) {
	...
	for _, p := range must {
		if _, err := os.Stat(p); err != nil {
			log.Printf("mail heal: %s eksik — sanalcp-mail-setup çalıştırılmamış olabilir", p)
		}
	}
